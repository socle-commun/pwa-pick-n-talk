  ...
  return (
    <HeadlessDialog open={open} onClose={close} className="lg:hidden">
      <HeadlessDialogBackdrop
        transition
        className={cn("fixed inset-0 bg-black/30 transition data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in")}
      />
      <HeadlessDialogPanel
        transition
        className={cn("fixed inset-y-0 w-full max-w-80 p-2 transition duration-300 ease-in-out data-closed:-translate-x-full")}
      >
        <div className={cn("flex h-full flex-col rounded-lg bg-white shadow-xs ring-1 ring-zinc-950/5 dark:bg-zinc-900 dark:ring-white/10")}>
