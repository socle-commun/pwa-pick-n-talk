...
export default function PictogramCard({ pictogram, className }: PictogramCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className={cn("w-24 h-24 mb-3 flex items-center justify-center")}>
        {imageUrl ? (
          <img src={imageUrl} alt={pictogram.word} className={cn("max-w-full max-h-full object-contain")} />
        ) : (
          <div
            className={cn(
      ...
      {isSpeechSynthesisSupported() && (
        ...
        >
          ...
          <span className={cn("sr-only")}>{isSpeaking ? "Speaking..." : `Speak "${pictogram.word}"`}</span>
        </Button>
      )}
    </div>
  );
