...
export default function BinderPage() {
  ...
  if (!uuid) {
    return (
      <div className={cn("flex items-center justify-center h-full p-4")}>
        <div className={cn("text-red-600 dark:text-red-400")}>Invalid binder ID</div>
      </div>
    );
  }
  ...
  if (!binder) {
    return (
      <div className={cn("flex items-center justify-center h-full p-4")}>
        <div className={cn("text-zinc-600 dark:text-zinc-400")}>Loading binder...</div>
      </div>
    );
  }
  ...
  return (
    <div className={cn("min-h-full")}>
      ...
      <div className={cn("bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-700 p-6")}>
        <h1 className={cn("text-3xl font-bold text-zinc-900 dark:text-zinc-100")}>{binder.title}</h1>
        {binder.description && <p className={cn("mt-2 text-zinc-600 dark:text-zinc-400")}>{binder.description}</p>}
      </div>
